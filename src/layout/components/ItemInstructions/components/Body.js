...
export function Index({  
    ...
    function handleSubmit(count){
        ...
        for(let i = 0; i < options.length; i++){
            const { isRequired, type, min } = options[i];
            if(!isRequired){
                continue;
            }
            if(!checkIfHidden(selectedOptions[i], type, min)){
                ...
                container.scroll({ top: option.offsetTop - 90, behavior: 'smooth' });
                return;
            }
        }
        ...
        >                            
            <div className="options">
                <Options
                    ...
                    onProperty={handlePropertyAvailable}
                    checkIfHidden={checkIfHidden}
                    onNote={event => setNote(event.target.value)}
                />                
            </div>
...
function getTotalPrice(selection, options){
    return selection.reduce( (outTotal, items, index) => { 
        const inner = items.reduce( (innerTotal, item, key) => {
            const { price } = options[index].items[key];
            if(item && price){ 
